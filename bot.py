...
async def find_similar(message: Message, state: FSMContext):
    """Поиск похожих артистов (с фильтром по русским исполнителям)"""
    ...
    # Получаем ID артиста и его жанр
    ...
    primary_genre = search_res[0].get("primaryGenreName", "").lower()
    
    # Определяем, русский ли артист (по наличию кириллицы в имени)
    is_russian_artist = any('а' <= c <= 'я' for c in artist_name.lower())
    
    # Получаем топ-треки и собираем коллабораторов
    tracks = await call_apple({"id": artist_id, "entity": "song", "limit": 30})
    
    similar = []
    seen = set()
    
    # Русские исполнители, которых будем предлагать
    russian_fallback = ["Король и Шут", "ДДТ", "Алиса", "Наутилус Помпилиус", "Агата Кристи", "Сплин", "Би-2", "Земфира", "Мумий Тролль", "Ночные Снайперы"]
    
    for track in tracks:
        feat_artist = track.get("artistName", "")
        if feat_artist.lower() == artist_name.lower():
            continue
        if feat_artist.lower() in seen:
            continue
        
        # Фильтр: если исходный артист русский, то и похожих ищем русских
        if is_russian_artist:
            has_cyrillic = any('а' <= c <= 'я' for c in feat_artist.lower())
            if not has_cyrillic:
                continue
        ...
        similar.append({
            "name": feat_artist,
            "url": f"https://music.apple.com/search?term={urllib.parse.quote(feat_artist)}"
        })
        if len(similar) >= 10:
            break
    
    # Если для русского артиста ничего не нашли — берем русский резерв
    if is_russian_artist and len(similar) < 5:
        for name in russian_fallback:
            if name.lower() not in seen and name.lower() != artist_name.lower():
                similar.append({
                    "name": name,
                    "url": f"https://music.apple.com/search?term={urllib.parse.quote(name)}"
                })
                if len(similar) >= 10:
                    break
    ...
    for i, a in enumerate(similar[:10], 1):
        result_text += f"{i}. <b>{html.escape(a['name'])}</b>\n🔗 <a href='{a['url']}'>Поиск в Apple Music</a>\n\n"
